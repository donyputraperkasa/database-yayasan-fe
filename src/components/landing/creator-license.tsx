...
export function CreatorLicense({ open, onClose }: CreatorLicenseProps) {
    ...
    return (
        ...
        <div
            ...
            className="relative w-full max-w-2xl max-h-[82vh] overflow-y-auto rounded-3xl border border-white/70 bg-white/92 p-6 text-[#1f2f46] shadow-2xl shadow-[#1f4f8f]/20 backdrop-blur-xl sm:p-7"
        >
            <button
            type="button"
            onClick={onClose}
            ...
            <p className="text-xs font-bold uppercase tracking-[0.35em] text-[#d89b00]">
                VERIFIED LICENSE
            </p>
            <h2 className="mt-3 bg-gradient-to-r from-[#123d78] via-[#1f4f8f] to-[#3a6cb7] bg-clip-text text-3xl font-extrabold tracking-tight text-transparent">
                MyBOPKRI
            </h2>
            <p className="mt-2 text-base font-medium text-[#55657f]">
            ...
            <div className="mt-3 rounded-2xl border border-[#dce8f7] bg-[#f7fbff] px-4 py-3 text-sm leading-6 text-[#44536b]">
            ...
            {licenseItems.map((item) => (
                <div
                key={item.label}
                className="rounded-2xl border border-[#e2eaf6] bg-[#f8fbff]/80 px-4 py-2.5"
                >
                <p className="text-xs font-medium uppercase tracking-wide text-[#7a8aa0]">
                    {item.label}
                </p>
                <p className="mt-1 text-sm font-semibold text-[#1f2f46]">{item.value}</p>
                </div>
            ))}
            </div>

            <div className="mt-4 rounded-2xl border border-[#f2d35f]/50 bg-[#fff8db]/70 px-4 py-4 text-sm leading-6 text-[#675b1f]">
