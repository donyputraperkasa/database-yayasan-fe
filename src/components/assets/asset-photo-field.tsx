...
type AssetPhotoFieldProps = {
  photoUrl?: string | null;
};

export function AssetPhotoField({ photoUrl }: AssetPhotoFieldProps) {
  const previewUrl = getMediaUrl(photoUrl);

  return (
    <label className="block md:col-span-2">
      <span className="text-sm font-semibold">Foto Aset</span>
      <div className="mt-2 flex flex-col gap-3 rounded-lg border border-[#ced9eb] bg-[#f8fbff] p-3 sm:flex-row sm:items-center">
        {previewUrl ? (
          ...
          <div className="grid h-20 w-24 place-items-center rounded-lg bg-white text-xs font-semibold text-[#748299]">
            Belum ada
          </div>
        )}
